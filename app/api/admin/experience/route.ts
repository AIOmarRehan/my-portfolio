...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      console.error('POST /api/admin/experience error', err instanceof Error ? `${err.message}\n${err.stack}` : String(err))
    } catch (logErr) {
...
export async function PUT(req: NextRequest) {
  try {
    ...
    const { data, error } = await supabase.from('experiences').update(updates).eq('id', body.id).select().single()
    if (error) {
    ...
    try {
      console.error('PUT /api/admin/experience error', err instanceof Error ? `${err.message}\n${err.stack}` : String(err))
    } catch (logErr) {
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    try {
      console.error('DELETE /api/admin/experience error', err instanceof Error ? `${err.message}\n${err.stack}` : String(err))
    } catch (logErr) {
